...
jsPsych.plugins["image-buttonkeyboard-response"] = (function () {

  var plugin = {}

  jsPsych.pluginAPI.registerPreload('image-buttonkeyboard-response', 'stimulus', 'image')

  plugin.info = {
    name: 'image-buttonkeyboard-response',
    description: '',
    parameters: {
      stimulus: {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    function after_response(choice) {
      ...
      if (isString(choice)) {
        ...
        response.answer = trial.choices[response.button]

      } else {
        // Then it's a key stroke
        response.rt = choice.rt
        response.key = choice.key
        response.answer = jsPsych.pluginAPI.convertKeyCodeToKeyCharacter(choice.key)
      }
      ...
      if (response.key == null && response.button == null) {
        response = choice
      }
    ...
    function end_trial() {
      ...
      var trial_data = {
        ...
        "response_type": (response.button == null) ? "Keyboard" : "Button"
      }
